...
class BetterLocalSearch:
    ...
    def flip_random_assignment_in_clause(self, clause):
        which = random.choice(clause)
    ...
    def solve(self, assignment_timeout, termprint=False, termprint_delay_seconds=1):
        ...
        for _ in range(assignment_timeout):
            if self.formula_satisfied():
                ...
                for clause in self.clauses:
                    if self.clause_satisfied(clause) is not None\
                        and not self.clause_satisfied(clause):
                        idx = self.flip_random_assignment_in_clause(clause)
                        break
                else:
                    assert False
                ...
                if termprint:
                    TerminalGraphics.flush_terminal()
                    TerminalGraphics.print_clauses(self.clauses, self.assignments)
                    print(f"flipped x{abs(self.assignments[idx])}")
                    time.sleep(termprint_delay_seconds)
        else:
            # timeout
            return None
